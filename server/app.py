...
# IMPORTANT: Monkey patch must be called before any other imports
import eventlet
eventlet.monkey_patch()
...
socketio = SocketIO(
    app,
    async_mode='eventlet',  # Change async_mode to eventlet to match the worker
    cors_allowed_origins=ALLOWED_ORIGINS,
    ping_timeout=60,
    ping_interval=25,
    logger=True,
...
# Global variables (per-client storage)
# NOTE: The global Jarvis_instance is replaced by a dictionary
jarvis_instances = {}
Jarvis_loop = None
Jarvis_thread = None
shutdown_flag = False
...
def cleanup_resources():
    """Clean up all resources"""
    global jarvis_instances, Jarvis_loop, Jarvis_thread
    
    logger.info("Cleaning up resources...")
    
    # Stop all Jarvis instances
    for sid, instance in list(jarvis_instances.items()):
        logger.info(f"Stopping Jarvis instance for SID: {sid}...")
        if Jarvis_loop and Jarvis_loop.is_running():
            try:
                future = asyncio.run_coroutine_threadsafe(instance.stop_all_tasks(), Jarvis_loop)
                future.result(timeout=10)
                logger.info(f"Jarvis tasks for {sid} stopped successfully.")
            except Exception as e:
                logger.error(f"Error stopping Jarvis tasks for {sid}: {e}")
        jarvis_instances.pop(sid)
...
@socketio.on('connect')
def handle_connect():
    """Handles new client connections, creates a new Jarvis instance for each"""
    global jarvis_instances, Jarvis_loop, Jarvis_thread
    ...
    # Start asyncio thread if not running
    if Jarvis_thread is None or not Jarvis_thread.is_alive():
        logger.info("Asyncio thread not running. Starting new loop and thread.")
        Jarvis_loop = asyncio.new_event_loop()
        Jarvis_thread = threading.Thread(target=run_asyncio_loop, args=(Jarvis_loop,), daemon=True)
    ...
    # Create new Jarvis instance for the new client
    if client_sid not in jarvis_instances:
        logger.info(f"Creating NEW Jarvis instance for SID: {client_sid}")
        if not Jarvis_loop or not Jarvis_loop.is_running():
            logger.error(f"Cannot create Jarvis instance, asyncio loop not ready for SID {client_sid}")
            emit('error', {'message': 'Assistant initialization error (loop).'}, room=client_sid)
        ...
        try:
            with app.app_context():
                new_instance = Jarvis(socketio_instance=socketio, client_sid=client_sid)
                jarvis_instances[client_sid] = new_instance
            future = asyncio.run_coroutine_threadsafe(new_instance.start_all_tasks(), Jarvis_loop)
            logger.info("Jarvis instance created and tasks scheduled.")
        except Exception as e:
            logger.error(f"ERROR initializing Jarvis for SID {client_sid}: {e}")
            emit('error', {'message': f'Failed to initialize assistant: {str(e)}'}, room=client_sid)
            jarvis_instances.pop(client_sid, None)
            return False
    else:
        logger.warning(f"Connection from existing SID: {client_sid}. Jarvis instance already exists.")
...
@socketio.on('disconnect')
def handle_disconnect():
    """Handles client disconnections and cleans up their Jarvis instance"""
    global jarvis_instances
    client_sid = request.sid
    logger.info(f"Disconnect from SID: {client_sid}")

    jarvis_instance = jarvis_instances.get(client_sid)
    if jarvis_instance:
        logger.info(f"Client {client_sid} disconnected. Attempting to stop Jarvis instance.")
        if Jarvis_loop and Jarvis_loop.is_running():
            try:
                future = asyncio.run_coroutine_threadsafe(jarvis_instance.stop_all_tasks(), Jarvis_loop)
                future.result(timeout=10)
        ...
        logger.warning(f"Disconnect from non-existent SID: {client_sid}. No action taken.")
...
def handle_text_message(data):
    ...
    if jarvis_instance:
        ...
        logger.warning(f"Jarvis instance not ready or session mismatch for text message from {client_sid}.")
        emit('error', {'message': 'Assistant not ready or session mismatch.'}, room=client_sid)
...
def handle_transcribed_text(data):
    ...
    jarvis_instance = jarvis_instances.get(client_sid)
    if transcript and jarvis_instance:
        if Jarvis_loop and Jarvis_loop.is_running():
            try:
                asyncio.run_coroutine_threadsafe(
                    jarvis_instance.process_input(transcript, is_final_turn_input=True), 
        ...
        logger.warning(f"Jarvis instance not ready or session mismatch for transcript from {client_sid}.")
...
def handle_video_frame(data):
    ...
    jarvis_instance = jarvis_instances.get(client_sid)
    if frame_data_url and jarvis_instance:
        if Jarvis_loop and Jarvis_loop.is_running():
            try:
                asyncio.run_coroutine_threadsafe(
                    jarvis_instance.process_video_frame(frame_data_url), 
...
def handle_video_feed_stopped():
    ...
    if jarvis_instance:
        ...
        logger.warning(f"Jarvis instance not ready or session mismatch for video_feed_stopped from {client_sid}.")
